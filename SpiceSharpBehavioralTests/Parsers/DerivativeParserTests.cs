...
namespace SpiceSharpBehavioralTests.Parsers
{
    ...
    public class DerivativeParserTests
    {
        ...
        protected void Check(Func<double[], double>[] reference, Lexer<IDerivatives<char, double>> parser, string expression)
        {
            // Add our variables
            ...
            {
                char id = (char)('a' + i);
                if (!variables.TryGetValue(id.ToString(), out var derivative))
                {
                    derivative = new Derivatives<char, double>
                    {
                        [id] = 1.0
                    };
                    variables.Add(id.ToString(), derivative);
                }
        ...
        {
            var parser = new Lexer<IDerivatives<char, double>>(new DerivativesParserParameters<char, double>(new DoubleParserParameters()));
