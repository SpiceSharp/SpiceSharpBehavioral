    ...
    public class DerivativesTests
    {
        ...
        public static IEnumerable<TestCaseData> Expressions
        {
            get
            {
                ...
                yield return new TestCaseData(Node.Voltage("v2", "v1"), new Dictionary<VariableNode, Node> { { v1, -one }, { v2, one } }).SetName("{m}(v1,v2)");
                yield return new TestCaseData(Node.Current("i1"), new Dictionary<VariableNode, Node> { { i1, one } }).SetName("{m}(iv1)");
                yield return new TestCaseData(Node.Plus(v1), new Dictionary<VariableNode, Node> { { v1, +one } }).SetName("{m}(+v1)");
                yield return new TestCaseData(Node.Minus(v1), new Dictionary<VariableNode, Node> { { v1, -one } }).SetName("{m}(-v1)");
                ...
                yield return new TestCaseData(Node.Subtract(v1, i1), new Dictionary<VariableNode, Node> { { v1, one }, { i1, -one } }).SetName("{m}(v1-i1)");
                yield return new TestCaseData(Node.Power(v1, v2), new Dictionary<VariableNode, Node>
                    {
                        { v1, v2 * Node.Power(v1, v2 - one) },
                ...
                yield return new TestCaseData(
                    ...
                    new Dictionary<VariableNode, Node> { { v1, Node.Constant("0.5") / Node.Function("sqrt", v1) } }).SetName("{m}(sqrt v1)");
                yield return new TestCaseData(
                    Node.Function("pwr", v1, v2),
                    new Dictionary<VariableNode, Node>
                    {
                        { v1, v2 * Node.Function("pwr", v1, v2 - one) },
